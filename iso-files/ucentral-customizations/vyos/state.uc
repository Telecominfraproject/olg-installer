...
//
// REVISED VERSION: Uses vyos-system-stats.py helper for structured JSON data collection
//
...
//
// Collect system/unit information from VyOS using the helper script
//
function collect_unit_info() {
	let unit = {};

	// Call vyos-system-stats helper script via VyOS API
	// This returns JSON with uptime, load, memory, and interface data
	let system_stats = vyos_api_call({
		op: "show",
		path: ["system", "stats"]
	});

	if (!system_stats || !system_stats.data) {
		printf("WARN: Failed to get system stats from vyos-system-stats helper\n");
		printf("INFO: Returning minimal unit info (helper script required)\n");

		// Return minimal data - helper script is required for full stats
		return {
			uptime: 0,
			localtime: time(),
			load: [0, 0, 0],
	...
	// Parse the JSON string returned by the helper
	let stats_data;
	try {
		stats_data = json(system_stats.data);
	} catch(e) {
		printf("ERROR: Failed to parse JSON from helper script\n");
		return {};
	}

	// Extract unit info from helper response
	unit.uptime = +(stats_data.uptime || 0);
	unit.localtime = time();

	// Load average
	if (stats_data.load && length(stats_data.load) >= 3) {
		unit.load = [
			+(stats_data.load[0]),
			+(stats_data.load[1]),
			+(stats_data.load[2])
	...
	if (stats_data.memory) {
		unit.memory = {
			total: +(stats_data.memory.total || 0),
			free: +(stats_data.memory.free || 0),
			cached: +(stats_data.memory.cached || 0),
			buffered: +(stats_data.memory.buffered || 0)
		};
	}
...
//
// Collect interface information from VyOS using helper script + config
//
function collect_interfaces(system_stats_data) {
	let interfaces = [];

	// Get interface statistics from helper script (already called in collect_unit_info)
	// If not passed, call it again
	let stats_data = system_stats_data;
	...
	if (!stats_data) {
		...
		if (system_stats && system_stats.data) {
			try {
				...
				printf("ERROR: Failed to parse JSON from helper\n");
				return interfaces;
			}
		} else {
			return interfaces;
	...
	// Get VyOS configuration for interface topology
	...
	// Process interface statistics from helper
	if (stats_data.interfaces && length(stats_data.interfaces)) {
		for (let if_data in stats_data.interfaces) {
			let if_name = if_data.ifname;

			// Skip loopback and special interfaces
			if (if_name == "lo" || index(if_name, "pim") >= 0)
				continue;

			let iface = {
				name: if_name,
				location: interface_roles[if_name]?.description || if_name
			};

			// Get IP addresses from the addresses array in helper output
			if (stats_data.addresses && length(stats_data.addresses)) {
				for (let addr_data in stats_data.addresses) {
					if (addr_data.ifname == if_name && addr_data.addr_info) {
						let ipv4_addrs = [];
						let ipv6_addrs = [];

						for (let addr_entry in addr_data.addr_info) {
							if (addr_entry.family == "inet") {
								// IPv4
								let addr_str = sprintf("%s/%d", addr_entry.local, addr_entry.prefixlen);
								push(ipv4_addrs, addr_str);
							} else if (addr_entry.family == "inet6" && addr_entry.scope == "global") {
								// IPv6 (only global scope)
								push(ipv6_addrs, {
									address: sprintf("%s/%d", addr_entry.local, addr_entry.prefixlen)
								});
							}
						}

						if (length(ipv4_addrs))
							iface.ipv4 = { addresses: ipv4_addrs };
						if (length(ipv6_addrs))
							iface.ipv6 = { addresses: ipv6_addrs };
					}
				}
			}

			// Extract counters from stats64 field
			let counters = {};
			if (if_data.stats64) {
			...
			iface.counters = counters;

			// Delta counters (inlined to avoid nested function calls)
			let prev_if = previous.interfaces && previous.interfaces[if_name];
			if (prev_if && prev_if.counters) {
				let deltas = {};
				for (let key in ["rx_packets", "tx_packets", "rx_bytes", "tx_bytes", "rx_errors", "tx_errors", "rx_dropped", "tx_dropped", "multicast", "collisions"]) {
					...
					let prev = prev_if.counters[key] || 0;
					let delta = curr - prev;
...
state.unit = collect_unit_info();  // Uses system_stats_data internally
state.interfaces = collect_interfaces(system_stats_data);
state["link-state"] = collect_link_state(system_stats_data, interface_roles);
